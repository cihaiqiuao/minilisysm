...
class RuleEngine {
  public:
    ...
    std::optional<InternalEvent> evaluate_process_presence(const std::string& name, bool present);
  ...
  private:
    ...
    InternalEvent make_process_event(const std::string& name, EventLevel level, EventStatus status,
                                     const RuleContext& context) const;
    ...
    std::unordered_map<std::string, RuleContext> process_health_;
};
