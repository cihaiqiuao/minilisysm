...
struct MetricLabel {
    ...
    MetricLabel(std::string key_value, std::string label_value)
        : key(std::move(key_value)), value(std::move(label_value)) {}
    std::string key;
    std::string value;
};
...
class MetricRegistry {
  ...
  private:
    ...
    void set_locked(const std::string& name, MetricKind kind, double value, std::vector<MetricLabel> labels, bool add);
