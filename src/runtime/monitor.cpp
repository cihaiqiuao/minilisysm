...
Monitor::Monitor(MonitorConfig config)
    : config_(std::move(config)),
      ...
      dispatcher_(std::make_unique<EventDispatcherGroup>(
          config_,
          event_queues_,
          StorageFactory::create_event_sinks(config_))),
      ...
      metrics_server_(std::make_unique<MetricsServer>(config_, [this]() {
...
std::string Monitor::render_metrics() const
{
    ...
    if (config_.metrics_scrape_runtime) {
        ...
        metrics_.set_counter("minilisysm_queue_dropped_total", static_cast<double>(queues.dropped_count), {{"queue", "source"}});
        metrics_.set_counter("minilisysm_queue_dropped_total", static_cast<double>(queues.sink_dropped_count), {{"queue", "sink"}});
        metrics_.set_gauge("minilisysm_queue_high_watermark", static_cast<double>(queues.high_watermark), {{"queue", "source"}});
        metrics_.set_gauge("minilisysm_queue_high_watermark", static_cast<double>(queues.sink_high_watermark), {{"queue", "sink"}});
        metrics_.set_counter(
            "minilisysm_dispatcher_sink_push_failures_total",
        ...
        if (dispatcher_) {
            for (const auto& item : dispatcher_->sink_stats()) {
                ...
                metrics_.set_counter("minilisysm_network_send_errors_total", static_cast<double>(stats.send_errors), label);
                metrics_.set_counter("minilisysm_network_retries_total", static_cast<double>(stats.retry_count), label);
                metrics_.set_gauge("minilisysm_network_wal_pending_events", static_cast<double>(stats.wal_pending_events), label);
                ...
                metrics_.set_counter(
                    ...
                    static_cast<double>(stats.wal_overflow_dropped_events),
                    label);
            }
        }
    }
    if (config_.metrics_scrape_collectors) {
        metrics_.set_counter("minilisysm_collector_failures_total", static_cast<double>(meminfo_failures_.load()), {{"collector", "meminfo"}});
        metrics_.set_counter("minilisysm_collector_failures_total", static_cast<double>(self_status_failures_.load()), {{"collector", "self_status"}});
        metrics_.set_counter("minilisysm_collector_failures_total", static_cast<double>(sched_delay_failures_.load()), {{"collector", "sched_delay"}});
        metrics_.set_counter("minilisysm_collector_failures_total", static_cast<double>(io_delay_failures_.load()), {{"collector", "io_delay"}});
        const SchedDelayCollectorRuntimeStats sched_stats = sched_delay_->runtime_stats();
        metrics_.set_counter("minilisysm_ebpf_ringbuf_drops_total", static_cast<double>(sched_stats.ebpf_ringbuf_drops));
        metrics_.set_counter("minilisysm_ebpf_allowlist_exec_seen_total", static_cast<double>(sched_stats.ebpf_allowlist_exec_seen));
        metrics_.set_counter("minilisysm_ebpf_allowlist_exit_cleaned_total", static_cast<double>(sched_stats.ebpf_allowlist_exit_cleaned));
        metrics_.set_counter("minilisysm_ebpf_allowlist_stale_hits_total", static_cast<double>(sched_stats.ebpf_allowlist_stale_hits));
        metrics_.set_counter("minilisysm_ebpf_aggregate_drops_total", static_cast<double>(sched_stats.ebpf_aggregate_drops));
        metrics_.set_gauge("minilisysm_ebpf_allowlist_scanned_processes", static_cast<double>(sched_stats.allowlist_scanned_processes));
        metrics_.set_gauge("minilisysm_ebpf_allowlist_matched_pids", static_cast<double>(sched_stats.allowlist_matched_pids));
        metrics_.set_gauge("minilisysm_ebpf_allowlist_matched_tids", static_cast<double>(sched_stats.allowlist_matched_tids));
        metrics_.set_gauge("minilisysm_ebpf_allowlist_refresh_elapsed_ms", static_cast<double>(sched_stats.allowlist_refresh_elapsed_ms));
    }
...
void Monitor::record_meminfo_metrics(const MeminfoSample& sample)
{
    ...
    metrics_.set_gauge("minilisysm_system_memory_available_bytes", static_cast<double>(sample.mem_available_kb) * 1024.0);
}
...
void Monitor::record_sched_delay_metrics(const SchedDelaySample& sample)
{
    ...
    metrics_.set_gauge("minilisysm_sched_wait_us", static_cast<double>(sample.delta_wait_sum_us), {{"pid", pid}, {"tid", tid}});
    metrics_.set_gauge(
        "minilisysm_sched_involuntary_switches",
        static_cast<double>(sample.delta_involuntary_switches),
        {{"pid", pid}, {"tid", tid}});
    metrics_.set_gauge("minilisysm_sched_max_wait_us", static_cast<double>(sample.max_wait_us), {{"pid", pid}, {"tid", tid}});
    metrics_.set_gauge("minilisysm_sched_avg_wait_us", static_cast<double>(sample.avg_wait_us), {{"pid", pid}, {"tid", tid}});
    metrics_.set_gauge("minilisysm_sched_aggregate_count", static_cast<double>(sample.aggregate_count), {{"pid", pid}, {"tid", tid}});
}
...
void Monitor::record_io_delay_metrics(const IoDelaySample& sample)
{
    ...
    metrics_.set_gauge("minilisysm_io_delta_count", static_cast<double>(sample.delta_io_count), {{"device", sample.device}});
}
